...
class checkerboard:
	...
	def get_color(self, x, z):
		...
		if math.fabs(math.fmod(x, 400.0)) >= 0.0 and math.fabs(math.fmod(x, 400.0)) <= 30.0:
			cf = 1

		if math.fabs(math.fmod(z, 400.0)) >= 0.0 and math.fabs(math.fmod(z, 400.0)) <= 30.0:
			cf = 1

		if cf == 1:
			return [1.0/255.0, 255.0/255.0, 255.0/255.0]
		else:
			return [1.0/255.0, 1.0/255.0, 75.0/255.0]
